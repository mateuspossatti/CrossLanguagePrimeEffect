...
class Keyboard:
    ...
    def waitKeys(self, stimDraw, maxWait=float('inf'), keyList=None, waitRelease=True, clear=True):
        ...
        # VERIFY THE QUANTITY OF KEYPRESS OBJECT. IF > 1 THAN THE MINIMUM POSITIVE RT IS CHOOSEN
        if len(keys) > 1:

            # PRINT WARNING INFORMATION
            print('WARNING. More than one key was recorded. Total: {} recorded\nThe key name and rt are shown below'.format(len))

            n = 0
            for key in keys:
                print('Key Position Order: {}. Key RT: {}. Key Name: {}'.format(n, key.rt, key.name))
                n += 1

            # CLEAN THE NEGATIVE RT
            keys_rt = {}
            n = 0
            for key in keys:
                # IF RT IS NEGATIVE OR 0 THAN PUT RT EQUAL TO INF
                if key.rt <= 0:
                    keys_rt[n] = float('inf')
                    n += 1

                # RT IS NOT <= 0 THAN ADD THE REAL RT IN THE DICTIONARY
                else:
                    keys_rt[n] = key.rt
                    n += 1

            # CHOOSE THE MINIMUM RT
            minVal = np.min(keys_rt.values())

            # CHOOSE RETURN THE KEY WITH THE LESS POSITIVE RT
            for indx in keys_rt.keys():
                if keys_rt[indx] == minVal:
                    return keys[indx]

        # IF IT'S ONLY ON KEYPRESS OBJECT THAN RETURN IT
        elif len(keys) == 1:
            return keys[0]

        # RAISE EXCEPTION IF THERE'S LESS THAN 1 KEYPRESS OBJECT
        else:
            raise Exception("The keys's length is less than 1")
